...
class Solution {
 public:
  vector<vector<int>> getSkyline(vector<vector<int>> &buildings) {
    vector<Event> events;
    events.reserve(buildings.size() << 1);
    int id = 0;
    ...
    for (const auto &e : events) {
      ...
      if (type == 1) {
        if (heap.Max() < h)
          res.emplace_back(initializer_list<int>{x, h});
        heap.Add(h, id);
      } else {
        heap.Remove(id);
        if (heap.Max() < h)
          res.emplace_back(initializer_list<int>{x, heap.Max()});
      }
    }
    return res;
  }
 private:
  struct Event {
    ...
    // Entering event sort h from large to small,
    // Leaving event sort h from small to large
    bool operator<(const Event &e) const {
      if (x == e.x) return type * h > e.type * e.h;
      return x < e.x;
    }
  };
  class MaxHeap {
   public:
    MaxHeap(int max_item): idx_(max_item, -1), size_(0), vals_(max_item) {}
    ...
    void Remove(int id) {
      int id_to_evict = idx_[id];
      SwapNode(id_to_evict, size_ - 1);
      --size_;
      HeapifyDown(id_to_evict);
    }
   ...
   private:
    ...
    void HeapifyUp(int i) {
      while (i != 0) {
        int p = (i - 1) / 2;
        if (vals_[i].first <= vals_[p].first) return;
        SwapNode(i, p);
        i = p;
      }
    }
    ...
    void HeapifyDown(int i) {
      while (true) {
        ...
        int c2 = (i << 1) + 2;
        if (size_ <= c1) return;
        int c = (c2 < size_ && vals_[c1].first < vals_[c2].first) ? c2 : c1;
        if (vals_[c].first <= vals_[i].first) return;
        SwapNode(c, i);
        i = c;
      }
    }
