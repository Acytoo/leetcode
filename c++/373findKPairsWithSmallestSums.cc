...
class Solution {
 public:
  vector<vector<int>> kSmallestPairs(vector<int>& nums1, vector<int>& nums2, int k) {
    const int n1 = min(k, static_cast<int>(nums1.size())),
              n2 = min(k, static_cast<int>(nums2.size()));
    if (!n1 || !n2) return {};
    multimap<int, vector<int>> m;

    for (int i=0; i<n1; ++i)
      for (int j=0; j<n2; ++j)
        m.emplace(nums1[i]+nums2[j], initializer_list<int>{nums1[i], nums2[j]});
...
class Solution_heap_buffer_overflow {
 public:
  vector<vector<int>> kSmallestPairs(vector<int>& nums1, vector<int>& nums2, int k) {
    const int m = min(k, static_cast<int>(nums1.size())),
              n = min(k, static_cast<int>(nums2.size()));
    vector<vector<int>> res;
    res.reserve(m * n);

    for (int i=0, m=min(k, static_cast<int>(nums1.size())); i<m; ++i)
      for (int j=0, n = min(k, static_cast<int>(nums2.size())); j<n; ++j)
  ...
  vector<vector<int>> kSmallestPairs(vector<int>& nums1, vector<int>& nums2, int k) {
    ...
    for (const auto n1: nums1)
      for (const auto n2: nums2) {
        q.emplace(initializer_list<int>{n1, n2});
        if (q.size() > k) q.pop();
      }
    vector<vector<int>> res;
    while (!q.empty()) {
      res.emplace_back(move(q.top()));
      q.pop();
    }
    return res;
  }
